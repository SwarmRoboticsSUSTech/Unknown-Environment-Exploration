    ...
    for i in range(len(frontiers_1)):
        ...
        if r_1 < prob_one_cluster: # update from self cluster
            if random.random() < 0.4:
                ...
                indi_1 = resampling(frontiers_1, total_weights[robotIndex])  # math.ceil(random.random() * (len(frontiers)-1))   
            ...
            indi_2 = resampling(frontiers_2, total_weights[cluster_2])    #math.ceil(random.random() * (len(frontiers_2)-1)) 
            indi_1 = resampling(frontiers_1, total_weights[robotIndex])        #math.ceil(random.random() * (len(frontiers)-1))
            if random.random() < 0.5:
                indi_temp = pick_two_cluster(centerfrontiers[robotIndex], centerfrontiers[cluster_2], robotlocation)
            else:
                indi_temp = pick_two_cluster(frontiers_1[indi_1], frontiers_2[indi_2], robotlocation)
        print("robotlocation", robotlocation.x, robotlocation.y)
        ...
        if(frontiers_1[i].weight < indi_temp.weight):
            ...
            calculate_frontiers(frontiers_1, robotIndex, centerfrontiers, total_weights)
    return frontiers_1
...
def pick_two_cluster(frontier_W_1, frontier_W_2, robotlocation_W):
    ...
    ff_distance = math.hypot((frontier_R_1.x - frontier_R_2.x), (frontier_R_1.y - frontier_R_2.y))
    print("ff_distance:", ff_distance)
    if ff_distance < reject_distance:
        if ff_distance != 0:
            indi_temp_R.x = frontier_R_1.x * (reject_distance * 2 / ff_distance)
            indi_temp_R.y = frontier_R_1.y * (reject_distance * 2 / ff_distance)
        else:
